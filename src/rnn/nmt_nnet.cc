...
static void ReadHeader(FILE* file, NmtNNetConfig* cfg, int* version_ptr) {
...
static NmtNNetConfig ReadConfig(const std::string& model_file) {
...
NmtNNet::NmtNNet(const Vocabulary& src_vocab, const Vocabulary& tgt_vocab,
                 const NmtNNetConfig& cfg, bool use_cuda,
...
NmtNNet::NmtNNet(const Vocabulary& src_vocab, const Vocabulary& tgt_vocab,
                 const std::string& model_file, bool use_cuda,
                 bool use_cuda_memory_efficient)
...
void NmtNNet::Init() {
  ...
  fprintf(stderr,
      ...
      " maxent_hash_size=%" PRId64", maxent_order=%d,"
      "vocab_size for source lang=%d, vocab size for target lang=%d,"
      "use_nce=%d\n",
      cfg.layer_size, cfg.layer_type.c_str(), cfg.layer_count, cfg.maxent_hash_size,
      cfg.maxent_order, src_vocab.size(), tgt_vocab.size(), static_cast<int>(cfg.use_nce));
  embeddings.resize(src_vocab.size() + tgt_vocab.size(), cfg.layer_size);
  if (cfg.layer_size) {
...
void NmtNNet::ApplyDiagonalInitialization(Real alpha) {
  rec_layer->GetWeights()->DiagonalInitialization(alpha);
}

void NmtNNet::Save(const std::string& model_file) const {
  if (
...
void NmtNNet::SaveCompatible(const std::string& model_file) const {
  FILE* file = fopen(model_file.c_str(), "wb");
...
void NmtNNet::ReLoad(const std::string& model_file) {
  FILE *file = fopen(model_file.c_str(), "rb");

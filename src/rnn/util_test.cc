...
TEST(UtilTest, InitNormal) {
  ...
  auto stat = getMeanVariance(kNumValues, random_values);
  EXPECT_NEAR(0.0, stat.first, 1.0);
  EXPECT_NEAR(1.0, stat.second, 1.0);
}

TEST(UtilTest, Clip) {
  ASSERT_EQ(0, Clip(std::numeric_limits<Real>::quiet_NaN(), 1.0));
  ASSERT_EQ(-1.0, Clip(-2.0, 1.0));
  ASSERT_EQ(1.0, Clip(2.0, 1.0));
}

TEST(UtilTest, ShrinkMatrix) {
  Eigen::Matrix<Real, 2, 2, Eigen::RowMajor> matrix;
  matrix << std::numeric_limits<Real>::quiet_NaN(),
      std::numeric_limits<Real>::infinity(),
      -std::numeric_limits<Real>::infinity(),
      kMaxValue;
  ShrinkMatrix(matrix, kMaxValue);
  ASSERT_EQ(0.0, matrix(0.0));
  ASSERT_NEAR(1.0, matrix(0, 1), kEps);
  ASSERT_NEAR(-1.0, matrix(1, 0), kEps);
  ASSERT_LT(0.0, matrix(1, 1));
}
